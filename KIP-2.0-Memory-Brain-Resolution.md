...
## Downstream integration round — 2026-09-24

Integrating `3251912` into both reference engines (anda-db `0312b51`, `6a67d60`) and the reference Brain (anda-brain `c9dfe6b`) produced issue lists in those repositories (`anda-db/debug/KIP_SYNC_TASKS.md` §13.5, `anda-brain/docs/KIP_SYNC_TASKS.md` §13.4) and a development plan for the draft vocabulary (`anda-db/debug/DRAFT_VOCABULARY_PLAN.md` §2). Their KIP-side items are resolved here; engine and Brain work stays in those repositories.

| # | Finding | Resolution |
| --- | --- | --- |
| C0 | Both engines passed `world-time.json` except its four `DEFINE` cases, which they skip without `draft_vocabulary`, so the fixture could not leave `pending_engine` | The `DEFINE` cases move to the new pending fixture `draft-vocabulary.json`; `world-time.json` is verified (anda-db `6a67d60`, 384 of the suite's 388 cases on both engines); the manifest records the verified head |
| C1 | `commitment_review` had no idempotency key, so every Maintenance cycle could raise the same due Commitment again | One `commitment_review` Activity per Commitment, `client_key` `commitment_review:<commitment id>:<due_at>`: a replay raises nothing, a new `due_at` raises it again (Profile §5.7, §17; Memory Interface §4; BrainMaintenance §17; package model hint; oracle `commitmentReviewKey`) |
| C2 | The attention cursor ordered only by `raised_seq`, and one commit can raise several items | Items are ordered by `(raised_seq, ref)`; the cursor is the last delivered position, so a page may end inside one sequence without losing the rest (Memory Interface §4, `kip-memory.schema.json`; oracle `attentionAfter` pages) |
| C3 | `strength_policy` was an `ArtifactPin` with no artifact format and no standard artifact, so `effective_strength` could not agree across engines | `StrengthPolicy` in `kip-cognitive-records.schema.json`; the standard `profiles/policy-strength-half-life-30d.json` (`kip:strength-half-life-30d`); the formula, the evaluation instant, before-anchor and unknown-pin rules in Profile §6.1 and Spec §59.1; MEM-030, oracle `effectiveStrength`, pending fixture `mnemonic-strength.json` |
| C4 | §14.2 required a compatible context for supersession but did not define it, and no engine case pinned it | §14.2 defines compatibility (actor, Proposition or subject+Predicate lineage, canonical context set) and routes a scope error to retraction; KML-036, oracle `supersessionCompatible`, pending fixture `supersession-scope.json` |
| C5 | Memory Interface did not say which `asserted_at` a revision without a captured source takes | Every revision takes it from `source_ref`; a host API without one captures the request as the source (Memory Interface §4) |
| C6 | `kip-projection.schema.json` forbade the `policy` member the engine suite read, and its `Slot` lacked members §47.3 listed | The basis is the one place for policy, `valid_at` and snapshot (§27.2, §47.3); `Slot` gains optional `subject`, `predicate_ref`, `uncertainty` and `explanation`; the suite case reads `?b.basis.policy.id` under a named policy instead of an engine's private default |
| C7 | §25.4 said a predecessor ends at the successor's effective start, while both oracles and both engines use its start on that line | §25.4 speaks of line starts and line ends, combined bound by bound |
| C8 | `cognitive-memory@2.0.0` is rewritten in place during the draft | Unchanged by design (Specification Status): names and digests freeze together at release; this round moves the digest again |
| K1 | §20.16 left the draft vocabulary's shape open where two engines must agree | Same-kind conflicts over every locked package; body members and required `description`; unconstrained omitted endpoints; `{ref, schema_environment_version}` result and no `CLIENT KEY`; the synthesized, Space-local draft package in `LIST SCHEMA PACKAGES` / `DESCRIBE PACKAGE`; `lineage_maps` for promotion; Capsule import requires an explicit mapping of used draft symbols; `review_schema:<ref>` (Spec §20.16, Capsule §41.7, Profile §5.9, KIPFormation, BrainMaintenance §31, syntax card) |
| K2–K3 | The `DEFINE` cases covered four behaviors | `draft-vocabulary.json` (23 cases: listing, description, exact references, a draft Concept Type, projection over a draft Predicate, conflicts, refused members, placement, parameters); SCHEMA-022 and the new SCHEMA-023 (promotion, Capsule mapping); manifest and suite README |
| K4 | The toolkit checked only three `DEFINE` members | `@ldclabs/kip-lang` 2.4.1 checks the full §20.16 member lists statically |
| K5 | No model covered the draft vocabulary's authority boundary | `formal/governance/check_draft_vocabulary.py`: V1–V7 over 16,476 states, five bug-injection modes |
...
K6 is closed. At anda-db `e70e275` both reference engines implement the draft vocabulary, context-compatible supersession and computed strength, and run the suite of `597db44`: all 423 cases in its 25 fixtures pass on both, none skipped (Rust `tests/conformance.rs`; kip-do `test/conformance.test.ts` under a Node shim, its workerd pool not run). `draft-vocabulary.json`, `supersession-scope.json` and `mnemonic-strength.json` drop `pending_engine`; the manifest records the verified head and no fixture is pending. Promotion, Capsule mapping of draft symbols and a Principal holding only `propose_schema` are covered by the engines' own tests, not by this suite; they remain SCHEMA-023 and GOV-031 vectors here. The Brain side (anda-brain `b174f50`) drafts vocabulary with `DEFINE`, queues `review_schema:<kind>:<ref>` and promotes through an owner API. The remaining KIP-side item is publishing `@ldclabs/kip-lang` 2.4.1.
