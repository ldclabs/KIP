...
The normative consolidation is `SPECIFICATION.md`. If this Architecture conflicts with the current Specification, **the Specification takes precedence**.
...
These questions were raised while this architecture was being designed. The consolidated `SPECIFICATION.md` has since resolved them; the original discussion is preserved below with each outcome noted.
