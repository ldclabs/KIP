...
面向实现 Brain Recall 的模型。[Recall 策略](./BrainRecall_CN.md)负责选择与解释；[Core 核心规范](../KIP-2.0-SPECIFICATION_CN.md)定义语义。使用只读执行路径。任何读取均不会改变记忆强度 (strength)、置信度 (confidence) 或评分 (grades)。
