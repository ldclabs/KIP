...
生命周期地位决定候选清单的次序：`adopted` 领先，`trialed` 与 `proposed` 须标注「未经证明」，`revoked` 只作为警示或反例出现——绝不作为推荐。在每一档，已评定的地位都排在自我报告的成功故事之前。
...
记忆排序可综合利用：任务相关性、语义相似度、记忆强度、显著性、程序效用分、现实时效性、Experience 结果极性、已评定结果地位及反例相关性。当上下文预算迫使取舍时，已评定地位是决胜信号：被世界实际兑现过的记忆先于仅仅与任务相似的记忆序列化。最终的涉真信念判定仍必须来自认识投影，而非排序得分。
