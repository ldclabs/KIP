...
同时度量：待处理与逾期的 `Commitment`、`due_at` 已到或已过的 armed `Watch`、`DerivationState.status` 为 `stale` 的制品、待执行生命周期裁决的 `Skill`、存在争议的信念槽位、被隔离的导入认知，以及 `retention.expires_at` 已过期的元素。先计数，再动作。
...
`task_family` 字段为必填项——用以声明评定该 Skill 效果的结果证据流（规范 §15.7）。对于无法明确声明评定流的模式，应当拒绝将其编译为 Skill；该类模式属于 Insight，而非程序性记忆。初始 `utility` 代表准入时的预期下注；在真实的客观结果到来之前，各项评定计票保持为空。

**生命周期流转是严格裁决，而非主观观点。** `proposed → trialed → adopted → revoked` 的状态迁移只能由确定性代码执行：读取该 Skill `task_family` 之下已评定的结果证据，记录为一条 `lifecycle_verdict` Activity 并附带一条受保护的 UPDATE 语句（规范 F.6）。你的职责是调度并如实记录裁决过程；绝不能仅因某项 Skill「感觉可行」就擅自晋升，行动者自身的成功陈述亦不能算作评定结果。采纳状态始终是暂定的——后果流将持续进行评定，一旦表现退化即降级重试——且撤销门槛绝不高于采纳门槛。

**已采纳的 Skill 不等同于执行权限。** `utility` 仅代表 `[0,1]` 区间内的程序实用性；系统调用权限始终由 Governance 独立管控。导入的外部 Skill 一律以 `proposed` 状态进入且本地评定记录为空，直至在本地环境中完成试用与评估。
...
| 待执行生命周期裁决的 Skill         | < 10       | 对已评定结果运行确定性裁决   |
...
25. Skill 生命周期仅能经由针对已评定结果的确定性裁决推进流转——绝不凭主观推断，也绝不采信行动者自身的成功陈述。
26. 撤销门槛绝不高于采纳门槛，采纳状态也绝不意味着评定的终结。
