...
哪些技能挣得了采纳——哪些又被撤销？（What has earned adoption — and what lost it?）
...
`task_family` 是 Skill 的**打分句柄 (scoring handle)**：即能够证明其有效或证伪其缺陷的结果证据流（规范 §15.7）。该字段为必填项——一个无法声明由哪条后果流来评定自身的模式，不属于程序性记忆，应当作为 Insight 存储。修改 `task_family` 是一次普通的、留有完整审计痕迹的变更；且裁决始终严格绑定其所实际评定的任务族，因此单纯修改标签绝无法继承既往的评定成绩。
...
每一次晋升或降级均是对已评定结果证据的确定性裁决（§14）——绝非作者单方断言、时间衰减，更非行动模型的主观判断。

**已采纳的 Skill ≠ 系统执行权限。** 权限依然受 Governance 状态管控。从外部导入的 Skill 以 `proposed`/inactive 进入，不携带任何生命周期地位（规范 §31.4），直至在本地环境中完成试用与审核。
...
各项计数是针对该 Skill 的 `task_family` 之下已评定结果证据的统计计票，由裁决与评定 Activity 负责维护——绝不采信行动模型的自我报告。`graded_count` 涵盖包括 `partial`、`aborted`、`unknown` 在内的全部已评定结果总数，因此 `success_count` 与 `failure_count` 两个具名计数无需刻意伪装成穷尽完备。
...
OutcomeRecord 附加于结果证据（规范 §15.7）之上，使现实后果具备可查询性：`task_family` 声明其所属的可比后果流，`outcome_status` 复用 Experience 的状态词汇表（`success | partial | failure | aborted | unknown`），可选的 `magnitude` 承载具体部署自定义的量级评估（`[0,1]`）。仪器的原始输出以原样保留在证据载荷中；本 Facet 则作为其上的结构化评定索引。
...
上述机制让系统能够更好地观察世界；后果通道则让现实世界得以反向检验系统：结果证据（规范 §15.7）携带 `OutcomeRecord` Facet，由仪器化组件（如遥测、验证器、测试装置、工具、人工审查）写入，并通过 `task_family` 与被评定的认知建立联结。
...
- 行动模型**严禁**写入评定其自身行动的结果证据；其陈述仅为 `agent_statement`，至多作为上下文引用。
- 一条结果证据**应当**通过溯源引用其所观测的那次决策（`action_gate` 或执行 Activity）；但评定关系按 `task_family` 进行联结，因此仪器端无需感知具体由哪些 Skill 订阅了该流。
- 任务族词汇表由具体部署策略定义；族名**应当**保持稳定、具备命名空间，且保持克制收敛以利于持续积累评定历史。
- 消费者必须核验所评定结果的起源链，并拒绝不符合策略要求的来源——通道通过可审计性提供保障，而非依赖密码学绝对不可伪造性。
...
`outcome_observation` Activity 记录摄取仪器写入结果证据的操作——inputs：（若可表达）被观测的行动或门控 Activity；outputs：产出的结果证据。其关联的执行主体为仪器化主体，绝非其行动正被评定的当事主体。

`lifecycle_verdict` Activity 记录针对后果流所执行的确定性评估——inputs：所依据的已评定结果证据；outputs：发生生命周期流转的目标 Skill；`parameters_digest` 锚定评估规则与对比基准，确保审计者可完全复算重现裁决结果。裁决是由确定性代码读取已记录结果所做出的计算，绝非模型的主观判断；任何缺乏对应裁决 Activity 的状态变更均不被视为合法的生命周期迁移，而只是一次无法解释的未受控改动。
...
巩固流程**必须**在提议时就附上 `task_family`，并**必须**拒绝产出没有任务族的 Skill：一个无法声明由哪条后果流来评定自身的模式，因缺乏被证伪的途径而属于 Insight，而非程序性记忆。
...
1. **确定性迁移。** 晋升与降级**必须**由读取已评定结果证据的确定性代码执行——绝非作者单方断言、时间衰减，更非行动模型的主观判断。Brain 负责提议、编译与叙述；它从不擅自晋升。
2. **对比性、可重算的采纳。** 试用裁决回答的是*表现是否比基准基线更优*，而非*过程是否看似顺利*。对比基准如何构造属于 Brain 策略；对比基准被严格记录在案则是 Profile 纪律：裁决 Activity **必须**使规则身份、对比基准与被评定结果集完全可恢复，并且**应当**可表达为一条关于该 Skill 的 Proposition + Assertion，使裁决本身进入可审计的主张图谱。
...
生命周期地位同样不随导入迁移：无论源状态如何，导入的 Skill 均以 `proposed` 状态进入，且本地评定统计初始为空。胶囊可携带源系统的历史结果作为具有参考价值的背景证据——但那属于外部记录，绝不能直接计入本地裁决。
