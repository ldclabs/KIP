...
规范性统合文档为 `SPECIFICATION.md`。若本架构设计文档与当前规范发生冲突，**以规范 (Specification) 为准**。
...
编译负责提议，现实负责晋升。技能的生命周期地位——`proposed → trialed → adopted → revoked`——是由其声明的任务族之下已评定的结果证据，经由被完整记录的确定性裁决所挣得，绝不由编译该技能的过程自行断言。
...
这些问题是在设计本架构时提出的。统合后的 `SPECIFICATION.md` 现已将其全部解决；原始讨论保留如下，并注明了各自的决议结果。
