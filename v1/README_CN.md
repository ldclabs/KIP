...
> **KIP 1.x 历史归档 — v1.0-RC11。** 本目录已冻结，供历史查阅与迁移参考；新开发请从[根目录的 KIP 2.0](../README_CN.md)开始。
>
> 迁移前快照：[提交 `31cf33f`](https://github.com/ldclabs/KIP/tree/31cf33f84450918432083d19c085fa47cb74d879)。此处保留旧版文档、胶囊、函数定义、MCP Server 和 Skill，并调整归档链接。下文实现与发布说明反映历史状态，不代表当前 v2 兼容性。CONTEXT 历史稿在快照时已包含部分 v2 术语；当前工具包位于根目录 `packages/`，面向 v2。
>
> 参阅[仓库路径变更](../migration/Repository-Layout_CN.md)与[部署迁移指南](../migration/KIP-2.0-Migration-from-1.x_CN.md)。
...
<p align="center">
  ...
  <a href="../SPECIFICATION_CN.md"><img src="https://img.shields.io/badge/next-v2.0--draft-orange.svg" alt="KIP 2.0 规范草案"></a>
...
> **KIP 2.0 规范草案 —— [从这里开始 →](../README_CN.md)**
> 信念由静态存储字段升级为针对竞争断言的显式认知投影；证据与溯源活动升格为头等元素；新增的主动性层使智能体能够基于状态差分而非定时轮询发起行动：[`Watch`](../profiles/CognitiveMemoryProfile-2.0_CN.md#511-watch守望监听)、[`action_gate`](../profiles/CognitiveMemoryProfile-2.0_CN.md#9-活动-activities)、[`LIST DEPENDENTS`](../SPECIFICATION_CN.md#635-list-dependents-列举依赖方)、[`PURGE PAYLOAD`](../SPECIFICATION_CN.md#606-载荷清除-payload-purge)。
...
KIP 2.0 现为[根目录的默认开发版本](../README_CN.md)，继续保留规范性草案状态。以下介绍保留为历史背景；最新状态以根目录文档为准。

KIP 2.0 将核心数据模型建立在五种基本元素之上——Concept、Proposition、Assertion、Evidence、Activity——彻底解耦「命题陈述的存在性」与「命题的真实性信念」。信念体现为对多个相互竞争的 Assertion 进行动态计算的显式认知投影（`BELIEF` / `BELIEF SLOT`），证据与溯源活动由元数据字段升格为头等元素，数据模式则统一纳入带版本且经摘要锁定的模式包。[从 1.x 迁移指南 →](../migration/KIP-2.0-Migration-from-1.x_CN.md)

同时，2.0 补全了智能体在非交互期间的主动认知机制：[`Watch`](../profiles/CognitiveMemoryProfile-2.0_CN.md#511-watch守望监听) 是基于已提交变更外壳（Change Envelope）进行增量匹配的持久化注意力状态，其中 silence 类在预期变更超时未发生时触发，使主动性建立在精确的状态差分而非盲目轮询之上，且触发仅产生关注而不授予权限；[`action_gate`](../profiles/CognitiveMemoryProfile-2.0_CN.md#9-活动-activities) 完整记录行动门控的显式裁决（`act`、`ask`、`defer` 或主动静默 `silence`）；[`LIST DEPENDENTS`](../SPECIFICATION_CN.md#635-list-dependents-列举依赖方) 支持在溯源根节点修订后精准发现下游派生认知，避免隐性失效（[§57.5](../SPECIFICATION_CN.md#575-修订与派生认知-revision-and-derived-cognition)）；[`PURGE PAYLOAD`](../SPECIFICATION_CN.md#606-载荷清除-payload-purge) 则在销毁原始观测字节的同时完整保留证据记录、摘要与引用拓扑，实现无损于溯源体系的数据最小化。[查看完整架构与规范 →](../README_CN.md)
...
| [🧩 2.0 认知记忆 Profile](../profiles/CognitiveMemoryProfile-2.0_CN.md) | Experience、Skill、Commitment 等记忆类型 |
| [🧠 2.0 Brain](../brain/README_CN.md) | 2.0 版的 Formation / Recall / Maintenance（[English](../brain/README.md)） |
| [🤖 `$self` / ⚙️ `$system`](../SelfInstructions_CN.md) | 单智能体精简提示词组合（[`$system`](../SystemInstructions_CN.md)） |
| [🗂 设计文档](../design/) | 十份分子系统的参考性文档 |
| [🔤 语法与 Schema](../grammar/) | 规范性 EBNF，以及[传输层 Schema](../schemas/) |
| [🧪 一致性测试套件](../conformance/KIP-2.0-Conformance-Tests.md) | 覆盖 13 个一致性 Profile 的 298 条可移植测试向量 |
| [🔬 形式化验证](../formal/README_CN.md) | Alloy 与 TLA+ 模型及其验证结论 |

可用的中文镜像见当前根目录文档索引；部分设计与测试说明仅有英文。
