...
> **KIP 1.x archive — v1.0-RC11.** This directory is frozen for historical reference and migration. New development starts with [KIP 2.0 at the repository root](../README.md).
>
> Pre-migration snapshot: [commit `31cf33f`](https://github.com/ldclabs/KIP/tree/31cf33f84450918432083d19c085fa47cb74d879). This archive retains legacy docs, capsules, function definitions, the MCP server and Skill, with adjusted links. Implementation and release statements below reflect historical conditions, not current v2 compatibility. The historical CONTEXT files already included some v2 terms at the snapshot; current packages remain at the root and target v2.
>
> See [repository path changes](../migration/Repository-Layout.md) and the [deployment migration guide](../migration/KIP-2.0-Migration-from-1.x.md).
...
<p align="center">
  ...
  <a href="../SPECIFICATION.md"><img src="https://img.shields.io/badge/next-v2.0--draft-orange.svg" alt="KIP 2.0 Draft Specification"></a>
...
> **KIP 2.0 is in draft — [start here →](../README.md)**
> Belief becomes an explicit projection over competing assertions instead of a stored field, evidence and provenance become first-class elements, and a proactivity layer lets a Brain act on a state differential rather than a timer: [`Watch`](../profiles/CognitiveMemoryProfile-2.0.md#511-watch), [`action_gate`](../profiles/CognitiveMemoryProfile-2.0.md#9-activities), [`LIST DEPENDENTS`](../SPECIFICATION.md#635-list-dependents), [`PURGE PAYLOAD`](../SPECIFICATION.md#606-payload-purge).
...
KIP 2.0 is now the [default development version at the root](../README.md) and remains a normative draft. The overview below is historical context; consult the root documents for current status.

KIP 2.0 rebuilds Core around five element kinds — Concept, Proposition, Assertion, Evidence, Activity — so that *this statement exists* and *this statement is believed* stop being the same fact. Belief becomes an explicit Epistemic Projection (`BELIEF` / `BELIEF SLOT`) over competing Assertions, evidence and provenance become first-class elements rather than metadata, and schema moves into versioned, digest-pinned packages. [Migrating from 1.x →](../migration/KIP-2.0-Migration-from-1.x.md)

It also gives a Brain the half of memory that runs when nobody is asking. [`Watch`](../profiles/CognitiveMemoryProfile-2.0.md#511-watch) is durable attention state evaluated against committed Change Envelopes — including *silence* watches that fire when nothing arrives in time — so proactivity is a state differential rather than a timer, and firing grants attention, never authority. [`action_gate`](../profiles/CognitiveMemoryProfile-2.0.md#9-activities) records what the gate decided: `act`, `ask`, `defer`, or deliberate `silence`. [`LIST DEPENDENTS`](../SPECIFICATION.md#635-list-dependents) makes the cognition compiled from a revised root discoverable instead of quietly stale ([§57.5](../SPECIFICATION.md#575-revision-and-derived-cognition)). And [`PURGE PAYLOAD`](../SPECIFICATION.md#606-payload-purge) destroys observed bytes while keeping the Evidence record, its digest and its citations — data minimization that costs no provenance. [The full picture →](../README.md)
...
| [🧩 Cognitive Memory Profile 2.0](../profiles/CognitiveMemoryProfile-2.0.md) | Experience, Skill, Commitment, and the rest of the memory types |
| [🧠 Brain 2.0](../brain/README.md) | Formation / Recall / Maintenance for 2.0 ([中文](../brain/README_CN.md)) |
...
Available Chinese mirrors are listed in the current root index; some design notes and test guides are English-only.
