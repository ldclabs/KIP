...
**针对一致性修订前草案的历史报告。** 生命周期模型已于 2026-09-06 替换，契约已扩展。以下结果作为历史保留，不作为新修订版本的全新证明。当前的范围与结果请参见 [CONSISTENCY-REPORT_CN.md](./CONSISTENCY-REPORT_CN.md)。

**[English](./REPORT.md) | [中文](./REPORT_CN.md)**

**日期**：2026-08-15；增补 2026-09-02 (套件 5–7)；最后一次全量运行 2026-09-02 (§12)
**目标**：`SPECIFICATION.md` (2.0-draft)、`profiles/CognitiveMemoryProfile-2.0.md` 及 `grammar/*.ebnf`
...
| 世界时间：继承、缺失起点的缺省规则、时间界限、`kip:memory-default` 优先级 (§21.13, §25.2–§25.5) | 是，有界穷举配对 + 采样三元组 | Python 模型检测（套件 8；新增于 2026-09-23，详见记忆-大脑修订记录） |
...
| 性质 | 含义 | 规范 / Profile |
| --- | --- | --- |
| I1 仅限归因 (AttributionOnly) | 裁决的 inputs（因而 Skill 的计算型 `GradingState` 视图）仅包含经由决策链接至应用了该技能的、且具备仪器起源的结果；仅观测到结果本身绝不改变任何计票 | 规范 §15.7, 不变量 37; Profile §6.2, §14 规则 7 |
| I2 计票前必有基线 (BasisBeforeCount) | 对处于试用/采纳状态 Skill 的裁决，其 `current_trial` 所选取的不可变 TrialRecord 基线必须先于所有被计入的结果 | 验证学习 §4 |
...
运行结果（`run.sh` 第 5 节）：规范模式下 **368,247 个状态，所有性质全部成立，所有三个见证轨迹均可达**（采纳；采纳后因单次关联失败被撤销；S1 被采纳而同族的 S2 保持未计分）。自评部署变体（`--self-graded`，210 万状态）同样成立。四个错误注入配置均产生其对应的反例：`--family-join`（2026-09-02 之前的旧设计：以任务族作为归因依据）因 S2 被 S1 的结果打分而违反 I1；`--skip-trial` 违反 I2；`--count-imported` 违反 I1 和 I6；`--no-gate`（接受行动主体自身的结果且不作标记）违反 I7。
...
套件 5 的 `--family-join` 模式复现了 2026-08-31 草案中的后果通道设计，当时直接使用 task family 作为结果与 Skill 之间的关联依据：在同一个任务族下包含两个 Skill 时，因应用 S1 的决策所产生的结果会同时给 S2 打分，且未链接至任何决策的结果也会同时给两者打分。重构后的新设计 —— 仅通过 `outcome_observation → action_gate` 链路进行精确归因，在试用开启时选定试用记录，任务族仅作为对比基线数据流 —— 在模型的所有交错执行下全部验证通过（I1–I7）。该模型正是复审意见 P0-2 的机械化形式验证。
