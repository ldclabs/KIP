...
存在则 `UPSERT` 更新，不要重复创建。重复提及不是噪声，而是**强化**（间隔/测试效应）：已有知识被再次确认时应增强它——递增 `evidence_count`、刷新 `last_observed`、上调 `confidence`（上限 `0.99`）。这是对 Maintenance 衰减的稳态反作用力——复现的事实保持强壮，从不复现的事实自然褪色。强化同样在**回忆确认**时触发（真正的测试效应）：当助手消息陈述了一条已记住的事实、而用户确认或据此行动时，按同样方式强化该事实。

```prolog
// 再确认时强化（先读 evidence_count 再递增）
UPSERT {
  CONCEPT ?pref {
