...
| recall（召回） | 任务/问题，或使用上次游标的 `attention` 模式；可选在某回执之后 (after) 及预算 | 相关的过往记录、不确定性及覆盖范围 —— 或自游标以来引发的注意力事项 |
| revise（修订） | 捕获的更正、变更或“你听错了”；已知时附带 `change_kind` | 保留历史的新认知理解 |
...
- 明确区分不同修订：`correction`（对方早先说错了）、`world_change`（当时属实，如今发生变迁 —— 查询历史时间依然返回旧取值）、`misrecorded`（记忆记录有误；绝不要记为对方推翻言论）。若不确定，使用 `unspecified`。
- 使用 `mode: "attention"` 轮询 `recall` 并保留返回的游标：提醒事项、到期的承诺以及被设防守望的静默正是通过此方式触达。注意力事项是促使深思的提示，绝非执行行动的授权。
