...
本文档定义了大脑（Brain）如何利用 KIP 2.0 及认知记忆 Profile 2.0 从实践经验中沉淀与学习。本文档不属于 KIP Core 的强制要求；规范性协议语义请参见 [SPECIFICATION_CN.md](../SPECIFICATION_CN.md)。
...
具备实用价值、已采纳或派生得出的认知，绝不能自动提升自身的系统执行权限。从“导入 Experience → 本地提议 Skill → 经试用在本地挣得采纳”，依然不代表自动获取了外部工具的调用权限。
...
协议级不变量收录于规范 §102，Profile 级不变量收录于 Profile §23，两者均统一汇总于注册表文档 `KIP-2.0-Invariants_CN.md`；本文档不再赘述。本文档补充的是专门针对学习机制的特有不变量：

1. 成功写入不等于完成学习。
2. 预测偏差不等于置信度。
3. 相似度不等于情境适用性。
4. 检索结果若不能改变未来计算，就不是功能性记忆。
5. 学习效果必须在行为层面可验证 —— 将带有相关记忆的表现与其消融状态进行对照。
