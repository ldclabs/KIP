...
* **受保护目标**：尝试删除或修改受保护的系统结构会返回 `KIP_3004`。受保护结构包括元类型（`$ConceptType`/`$PropositionType`）、核心领域（如 `CoreSchema`）、系统行动者（`$self`/`$system`）的身份元组（`type` + `name`）及其 `core_directives`。`$self` 的普通可演化属性不受此规则保护。
...
**语法**：`SEARCH CONCEPT|PROPOSITION "<term>"|:term [WITH TYPE "<Type>"|:type] [LIMIT N|:limit]`
...
// 语义搜索：查找与“缓解头痛”相关的概念（具体实现视实现层而定）
...
2. **`execute_kip_readonly`**: 仅用于执行安全的只读查询命令（包含 KQL FIND/SEARCH、META DESCRIBE）。当 Agent 明确只需要检索知识而不会进行任何更改时，应优先使用此函数。
...
| **`commands`**   | Array   | 否       | 用于批量执行的 KIP 命令数组。**与 `command` 互斥**。数组元素可以是 `String`（使用共享 `parameters`）或 `Object`（`{command, parameters}`，独立参数会覆盖共享参数）。命令按顺序执行。**错误中断规则**：任何 KML（`UPSERT`/`DELETE`）错误会立即中断批次执行，以避免后续命令在不一致的图状态上运行；KQL（`FIND`/`SEARCH`）与 META（`DESCRIBE`）错误是隔离的只读失败，会内联返回并继续后续命令。 |
...
当使用 `commands`（批量执行）时，响应中的 `result` 是一个按命令顺序对应的数组。**一旦遇到 KML（`UPSERT`/`DELETE`）错误即停止执行**，因此数组长度可能小于提交的命令数。
...
**KML 错误时停止执行**：
```js
// 如果第 2 条命令是 KML（`UPSERT`/`DELETE`）失败：
{
  "result": [
    { "result": { ... } },                    // 第 1 条命令成功
...
| `KIP_3004` | `ImmutableTarget`     | 尝试修改或删除受保护的系统结构：元类型（`$ConceptType`、`$PropositionType`）、核心域（如 `CoreSchema`）、系统行动者（`$self`、`$system`）的**身份元组**（`type` + `name`）或其 `core_directives`。 | **禁止操作。** 注意：`$self` 的普通属性（如 `persona`、`strengths`、`behavior_preferences`、`identity_narrative`）被有意设计为可演化的，**不受**此限制约束。 |
| **4xxx**   | **系统与执行错误**    |                                                                                                                                                                                                    |                                                                                                                                                              |
