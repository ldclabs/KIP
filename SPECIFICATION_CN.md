...
| v1.0-RC6    | 2026-04-25 | v1.0 Release Candidate 6：澄清聚合查询的隐式 `GROUP BY` 语义、路径操作符 `{0,n}` 零跳语义、`WITH METADATA` 优先级、`DELETE CONCEPT` 级联语义、`KIP_3004` 保护范围、`OPTIONAL` 空值投影、`expires_at` 生命周期与批量执行 KQL/KML 错误语义 |
...
**关于我们**：
[ICPanda](https://panda.fans/)：ICPanda 是一个社区驱动项目，致力于构建基础设施和应用，使 AI Agent 能作为 Web3 生态中的一等公民持续发展。
...
*   **记忆生命周期（`expires_at`）**：非空的 `metadata.expires_at` 声明的是知识**何时**成为遗忘的候选，并**不会**自动把该条知识从查询结果中过滤掉——已过期的知识在被后台系统进程（通常由 `$system` 在睡眠周期中执行）真正清理或归档之前，仍然可被查询到。需要忽略已过期记忆的 Agent 应显式添加 `FILTER(IS_NULL(?x.metadata.expires_at) || ?x.metadata.expires_at > <now>)`。

### 2.11. 系统维护元数据与乐观并发控制

一个被多个写入者共享的记忆大脑（例如多个业务智能体写入同一个认知中枢，或 Formation 与睡眠周期并发运行）需要两项作者自述元数据无法提供的保证：**可信的簿记**（实际改了什么、实际在被使用什么）与读-改-写流程的**丢失更新保护**。KIP 通过保留的 `_` 元数据命名空间同时提供这两者。
...
| `_accessed_at`  | String | 可选         | 元素最近一次作为 `FIND` 或 `SEARCH` 顶层结果被返回的时间（ISO 8601）（模式匹配的中间绑定与 `dry_run` 不计入）。                                                |
| `_access_count` | Number | 可选         | 元素作为 `FIND` 或 `SEARCH` 顶层结果被返回的总次数。与 `_accessed_at` 一起，为记忆代谢进程提供*真实的*使用信号（“用进废退”），而非由作者手工维护的近似值。     |
| `_score`        | Number | 可选         | **瞬态字段，永不持久化。** 附加在 `SEARCH` 返回元素上的归一化相关度评分 `[0, 1]`（见 §5.2）。在搜索结果之外不存在。                                            |

引擎**可以**（MAY）定义额外的 `_` 前缀字段；Agent **必须**将未知的 `_` 字段视为只读，且**不得**依赖其存在。
...
KML 是 KIP 中负责知识演化的部分，是 Agent 实现学习的核心工具。它由四条语句组成：`UPSERT`（按身份定位的创建或更新）、`UPDATE`（基于模式匹配的批量变更）、`MERGE`（原子实体合并）与 `DELETE`（定向删除）。
...
* **级联语义**：所有以该节点为 `subject` 或 `object` 的命题链接都会被一同删除；若这些命题本身又被**高阶命题**引用（作为其主语或宾语），则会被递传地删除。这保证了 `DETACH` 后不会遗留悬空引用。实现应在响应中报告级联删除的数量，以便 Agent 审计影响。
...
* **语义（原子执行）**：
    1.  **重指链接**：所有以 `?source` 为主语或宾语的命题链接被重指向 `?target`，**保留链接的 `id`**（高阶引用保持有效）。若重指后与 `?target` 已有链接在 (Subject, Predicate, Object) 唯一性约束（§2.10）下冲突，则保留目标侧的链接：源侧链接缺失的属性/元数据键被补入（已有键以目标侧为准），指向被弃链接的高阶引用被重指到幸存链接，重复链接随之移除。
...
**功能**：`SEARCH` 命令用于将自然语言术语链接到知识图谱中明确的实体。它是协议的**联想检索**原语：查找由索引驱动（文本和/或向量），而非完整的图模式匹配。Agent 的回忆很少从精确的名字出发——它从*意思*出发——因此语义检索在此被规范为一等的、可移植的能力，而不是实现层的脚注。
...
| `"semantic"`| 在接地字段上做基于意义的相似度检索。引擎负责向量的生成与存储；**向量永远不跨越协议边界**——Agent 发送文本，引擎解析意义。       |
...
* **胶囊内容**：
    ...
    * 保留的 `_` 元数据（`_version`、`_access_count` 等）**绝不导出**——那是源引擎的簿记，不是知识。
...
2. **`execute_kip_readonly`**: 仅用于执行安全的只读查询命令（KQL `FIND`、META `DESCRIBE` / `SEARCH` / `EXPORT`）。当 Agent 明确只需要检索知识而不会进行任何更改时，应优先使用此函数。
...
| **`commands`**   | Array   | 否       | 用于批量执行的 KIP 命令数组。**与 `command` 互斥**。数组元素可以是 `String`（使用共享 `parameters`）或 `Object`（`{command, parameters}`，独立参数会覆盖共享参数）。命令按顺序执行。**错误中断规则**：任何 KML（`UPSERT`/`UPDATE`/`MERGE`/`DELETE`）错误会立即中断批次执行，以避免后续命令在不一致的图状态上运行；KQL（`FIND`/`SEARCH`）与 META（`DESCRIBE`/`EXPORT`）错误是隔离的只读失败，会内联返回并继续后续命令。 |
...
当使用 `commands`（批量执行）时，响应中的 `result` 是一个按命令顺序对应的数组。**一旦遇到 KML（`UPSERT`/`UPDATE`/`MERGE`/`DELETE`）错误即停止执行**，因此数组长度可能小于提交的命令数。
...
* `_accessed_at`: `String` ISO 8601（可选），元素最近一次作为 `FIND`/`SEARCH` 顶层结果被返回的时间。
* `_access_count`: `Number`（可选），元素作为顶层结果被返回的次数。**这两个字段是引擎实测的“记忆激活度”信号**：维护进程在决定衰减、归档与地标提升时，应优先采用它们，而非作者手工维护的近似值。
