...
* `ORDER BY <expr> [ASC|DESC]`: 根据**单一排序表达式**对结果进行排序，默认为 `ASC`（升序）。表达式可以是已绑定变量（`?var`）、点表示法路径（`?var.attributes.<key>`），或同时出现在 `FIND` 中的聚合表达式（如配合隐式分组的 `ORDER BY COUNT(?n) ASC`）。
...
2. **`execute_kip_readonly`**: 仅用于执行安全的只读查询命令（KQL `FIND`、META `DESCRIBE` / `SEARCH`）。当 Agent 明确只需要检索知识而不会进行任何更改时，应优先使用此函数。
