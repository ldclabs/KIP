...
| 参数名                 | 类型    | 是否必须 | 描述                                                                                      |
| :--------------------- | :------ | :------- | :---------------------------------------------------------------------------------------- |
| **`command_language`** | String  | 是       | 指明本次命令使用的语言。有效值为: `"KQL"`, `"KML"`, `"META"`。                            |
| **`command_text`**     | String  | 是       | 包含完整、未经修改的 KIP 命令文本。使用多行字符串以保持格式和可读性。                     |
| **`parameters`**       | Object  | 否       | 一个可选的键值对对象，用于传递命令文本之外的执行上下文参数，命令文本中用 `$key_name` 占位 |
| **`dry_run`**          | Boolean | 否       | 如果为 `true`，则仅验证命令的语法和逻辑，不执行或持久化任何变更。                         |

**`parameters` 对象的可选键**:

*   **`domain` (String)**: 如果认知中枢的知识是多领域的，用于指定本次操作的目标领域。
...
| 键           | 类型   | 是否必须 | 描述                                                                                      |
| :----------- | :----- | :------- | :---------------------------------------------------------------------------------------- |
| **`status`** | String | 是       | 响应状态，必定为 `"ok"` 或 `"error"` 之一。                                               |
| **`data`**   | Object | 否       | 当 `status` 为 `"ok"` 时**必须**存在。包含请求的成功结果。其内部结构由 KIP 请求命令定义。 |
| **`error`**  | Object | 否       | 当 `status` 为 `"error"` 时**必须**存在。包含结构化的错误详情。                           |
| **`meta`**   | Object | 否       | 可选。包含关于本次交互的元数据，如执行用时、事务ID等，用于调试和监控。                    |
