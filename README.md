...
7. **An engineering-grade substrate.** Engine-maintained `_version` with `EXPECT VERSION` optimistic locking for multi-writer brains; `_access_count` as ground-truth activation for "use it or lose it"; bulk `UPDATE` arithmetic (a whole decay pass in one command); atomic `MERGE` entity consolidation; specified `keyword | semantic | hybrid` retrieval with normalized scores. ([§2.11](./SPECIFICATION.md#211-system-maintained-metadata--optimistic-concurrency), [§4.3–4.4](./SPECIFICATION.md#43-update-statement), [§5.2](./SPECIFICATION.md#52-search-statement))
