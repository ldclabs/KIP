...
The [Rust](https://github.com/ldclabs/anda-db/tree/main/rs/anda_cognitive_nexus) and [Cloudflare Durable Object](https://github.com/ldclabs/anda-db/tree/main/ts/kip-do) engines target KIP 2.0 drafts. Their shared suite now lives here as the [executable engine suite](./conformance/engine-suite/README.md); both pass every case of the current suite, including the draft vocabulary, supersession scope, computed strength and ingestion batch fixtures (anda-db `985451b`), so no fixture is `pending_engine`. The protocol remains `2.0-draft`, independently of tooling versions, and no real Brain learning result is claimed by structural tests.
