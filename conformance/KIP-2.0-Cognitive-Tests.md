...
The 30 vectors below supplement the parent suite. The original vectors remain binding except where their acceptance text is explicitly revised in the parent suite. Historical test reports do not attest to this revision.
...
## KIP2-MEM-030 — Computed strength under a pinned policy

A Concept's `MnemonicState` has base `memory_strength` 0.8, anchor `last_metabolized_at` and `strength_policy` pinned to `kip:strength-half-life-30d` with its digest. `effective_strength` is the base times `2^(−(t − anchor) / 30 days)` at the instant `t` the read is evaluated, never at `FOR TIME`; before its anchor it is the base. With the base, the anchor or the pin missing, with an unknown policy, or with a pin whose digest does not match, it is `null` — never `0.5` or another policy. Reading it writes nothing, and a write to it fails `ConstraintViolation` (Specification §18.2, §59.1; Profile §6.1). Oracle: MEM-030a–c. Engine fixture: `engine-suite/mnemonic-strength.json`.
