...
覆盖范围说明约束、承诺、依赖项、失败、经验、技能与证据。每一项的状态为 `complete`、`incomplete` 或 `not_applicable`。后者要求权威判定该通道在此范围内无关/不存在；缺少支持或不完整的遍历不能重新标记为 `not_applicable`。通道无法提供服务的保留内容会导致覆盖范围不完整。未公布某个等级本身不能证明对应通道不存在：已有 Space、原始 KIP 工具或 Capsule 导入可能留下相关内容。基础部署也必须依据权威的作用域内缺失或无关判定才能返回 `not_applicable`；无法提供相关保留内容时应返回 `incomplete`。这些保证针对的是经授权的已记录全集，绝非声称已检索了整个世界。
